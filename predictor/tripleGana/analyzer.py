# analyzer_triplegana.py — Analyzer optimizado para Triple Gana con todas las mejoras
import sqlite3
...
# -------------------------
def test_entropia_secuencia(secuencia: np.ndarray, num_categories: int = 1000) -> Dict[str, Any]:
    """Calcula la entropía de Shannon de una secuencia (0-999 para Triple Gana)"""
    try:
...
# -------------------------
# PREPARACIÓN (VECTORIZADA PARA 3 DÍGITOS)
# -------------------------
def preparar_datos(df: pd.DataFrame, config: TripleganaConfig = CONFIG) -> Optional[pd.DataFrame]:
    """Prepara y limpia los datos para análisis (Triple Gana: 0-999)"""
    ...
    # Vectorizar extracción de 3 dígitos (zfill(3) para Triple Gana)
    def extraer_digitos_vectorizado(serie: pd.Series) -> pd.DataFrame:
        """Extrae dígitos de manera vectorizada (3 dígitos para Triple Gana)"""
        str_nums = serie.apply(lambda x: f"{int(x):03d}")  # Cambiado a :03d para 3 dígitos
        digits = str_nums.str.extract(r'(\d)(\d)(\d)').astype(int)
        digits.columns = ['primer_digito', 'segundo_digito', 'tercer_digito']
        return digits
    ...
    # Último dígito
    df['ultimo_digito'] = df['tercer_digito']
    ...
    # Rangos por cien (dinámico, defensivo) - Ajustado para 0-999
    try:
        min_num = int(df['numero_int'].min())
        max_num = int(df['numero_int'].max())
        # Para Triple Gana, rangos de 100 en 100 son más significativos
        min_bin = (min_num // 100) * 100
        top = ((max_num // 100) + 1) * 100
        if top <= min_bin:
            top = min_bin + 100
        bins_fijos = list(range(min_bin, top + 1, 100))
        labels_fijos = [f'{bins_fijos[i]:03d}-{bins_fijos[i+1]-1:03d}' 
                       for i in range(len(bins_fijos)-1)]
        df['rango_cien'] = pd.cut(df['numero_int'], bins=bins_fijos, 
                                 labels=labels_fijos, right=False, include_lowest=True)
        df['rango_cien'] = df['rango_cien'].cat.add_categories(['out_of_range']).fillna('out_of_range')
    except Exception as e:
        logger.exception("Error creando rango_cien dinámico: %s", e)
        df['rango_cien'] = 'out_of_range'

    # También mantener rango_mil por compatibilidad
    df['rango_mil'] = '0-999'  # Para Triple Gana, todo está en un solo rango de mil
...
def test_normalidad(df: pd.DataFrame, config: TripleganaConfig = CONFIG) -> Dict[str, Any]:
...
# -------------------------
def crear_dataset_ml(df: pd.DataFrame, config: TripleganaConfig = CONFIG) -> Optional[Dict[str, Any]]:
    """
    ...
    try:
        ...
        elif config.ml_target == 'rango_cien':
            # Convertir categorías a códigos numéricos
            if 'rango_cien' not in df.columns:
                logger.error("rango_cien no existe en el DataFrame")
                return None
            le = LabelEncoder()
            y = le.fit_transform(df['rango_cien'].astype(str))
            problem_type = 'classification'
            n_classes = len(le.classes_)
            logger.info(f"Target: rango por cien, {n_classes} clases")
            
        elif config.ml_target == 'rango_mil':
            # Para Triple Gana, solo hay un rango (0-999)
            y = np.zeros(len(df), dtype=int)
            problem_type = 'classification'
            n_classes = 1
            logger.info(f"Target: rango por mil (solo 0-999 para Triple Gana)")
            
        elif config.ml_target == 'numero_int':
            y = df['numero_int'].astype(int)
            problem_type = 'regression'
            n_classes = None
            logger.info(f"Target: número entero completo (0-999, regresión)")
...
def analisis_completo(df: pd.DataFrame, config: TripleganaConfig = CONFIG) -> Dict[str, Any]:
    ...
    out['entropia'] = test_entropia_secuencia(df['numero_int'].values, num_categories=1000)  # 0-999
    
    # 3. Tests de independencia temporal
    if sm_modules_available:
        out['ljung_box'] = test_ljung_box(df['numero_int'].values)
    
    # 4. Análisis por posición de dígito (CRÍTICO) - Solo 3 dígitos para Triple Gana
    out['analisis_digitos'] = {}
    posiciones = ['primer_digito', 'segundo_digito', 'tercer_digito']
    ...
    if config.export_summaries:
        ...
        tests_df.to_csv(Path(config.output_dir) / 'triplegana_tests_summary.csv', index=False)
        ...
        _save_json(out, Path(config.output_dir) / 'triplegana_tests_completos.json')
...
def generar_reporte_ejecutivo(analisis: Dict, config: TripleganaConfig) -> Dict:
...
# -------------------------
# VISUALIZACIONES (12 gráficas adaptadas para Triple Gana)
# -------------------------
def visualizar(df: pd.DataFrame, analysis: Dict[str, Any], config: TripleganaConfig = CONFIG):
    """Visualizaciones completas con 12 gráficas adaptadas para Triple Gana"""
    ...
    # 1. Distribución de números (0-999)
    ...
    ax1.set_title('Distribución de números (Triple Gana: 0-999)', fontweight='bold')
    ax1.legend()
    ...
    ax2.set_xlabel('Hora')
    ax2.set_ylabel('Número (0-999)')
    
    # 3-5. Distribución por posición de dígito (3 dígitos)
    posiciones = ['primer_digito', 'segundo_digito', 'tercer_digito']
    ...
    # 6. Distribución por rangos de 100
    ax6 = plt.subplot(3, 4, 6)
    if 'rango_cien' in df.columns:
        rango_counts = df['rango_cien'].value_counts().sort_index()
        rango_counts = rango_counts[rango_counts.index != 'out_of_range']
        if not rango_counts.empty:
            sns.barplot(x=rango_counts.index.astype(str), y=rango_counts.values, ax=ax6, palette='viridis')
            ax6.set_title('Distribución por rangos de 100', fontweight='bold')
            ax6.set_xlabel('Rango (0-999)')
    ...
    stats_text = f"""
    Estadísticas Clave (Triple Gana):
    --------------------------------
    Muestras: {len(df):,}
    Rango: 0-999
    ...
    plt.suptitle('ANÁLISIS COMPLETO TRIPLE GANA (0-999) - DASHBOARD AVANZADO', 
                fontsize=16, fontweight='bold', y=1.02)
    plt.tight_layout()
    
    # Guardar figura
    out_path = Path(config.figures_dir) / 'triplegana_dashboard_avanzado.png'
...
def visualizar_digitos_detalle(df: pd.DataFrame, analysis: Dict[str, Any], config: TripleganaConfig):
    """Visualización especializada para análisis de dígitos por posición (3 dígitos)"""
    posiciones = ['primer_digito', 'segundo_digito', 'tercer_digito']
    ...
    plt.suptitle('ANÁLISIS DETALLADO DE DÍGITOS POR POSICIÓN - TRIPLE GANA', 
...
def main():
    """Función principal"""
    logger.info("=" * 60)
    logger.info("INICIANDO ANALYZER TRIPLE GANA CON ML")
    logger.info("=" * 60)
    ...
    print("\n" + "=" * 60)
    print("REPORTE EJECUTIVO - TRIPLE GANA (0-999)")
    print("=" * 60)
