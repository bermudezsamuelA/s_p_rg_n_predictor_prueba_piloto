# extractor_triplegana.py — Extrae resultados de Triple Gana con robustez y logging
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('triplegana_extractor.log'),
    ...
    for intento in range(max_reintentos):
        try:
            respuesta = requests.get(
                url,
                timeout=10,
                headers={'User-Agent': 'Mozilla/5.0 (compatible; TripleGanaBot/1.0)'}
            )
            respuesta.raise_for_status()
            
            if "<th" not in respuesta.text:
...
def extraer_datos(fecha_inicio=None, fecha_fin=None, pausa=2):
    ...
    logging.info(f"Extracción Triple Gana desde {fecha_inicio:%d/%m/%Y} hasta {fecha_fin:%d/%m/%Y}")
    ...
    if fechas_con_error:
        pd.DataFrame(fechas_con_error, columns=['fecha']).to_csv('errores_triplegana.csv', index=False)
        logging.warning(f"Errores exportados a errores_triplegana.csv")
...
def main():
    """CLI para ejecutar el extractor de Triple Gana"""
    import argparse
    parser = argparse.ArgumentParser(description='Extractor de resultados de Triple Gana')
