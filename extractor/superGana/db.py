...
DB_PATH = os.path.join(BASE_DIR, 'data', 'supergana.db')

def crear_tabla():
    """Crea la tabla específica para Super Gana"""
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    
    cursor.execute('''
        CREATE TABLE IF NOT EXISTS supergana_resultados (
    ...
    cursor.execute('CREATE INDEX IF NOT EXISTS idx_supergana_fecha_hora ON supergana_resultados(fecha, hora)')
...
def guardar_resultado(fecha, hora, numero, signo):
    ...
    try:
        cursor.execute('''
            INSERT OR IGNORE INTO supergana_resultados 
